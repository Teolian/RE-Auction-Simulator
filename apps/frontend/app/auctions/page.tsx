...
export default function AuctionsList() {
  ...
  const loadAuctions = async () => {
    try {
      ...
      setError(t('auctionsList.failedToConnect'))
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="flex items-center justify-center py-16">
        <div className="text-center">
          <div className="text-16 text-gray-600">{t('auctionsList.loadingAuctions')}</div>
        </div>
      </div>
    )
  }
    ...
    <>
      ...
      <div className="mb-8">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h1 className="text-32 font-bold text-gray-900 mb-2">{t('auctionsList.title')}</h1>
          ...
          >
            ...
            {t('auctionsList.createNewAuction')}
          </Link>
        </div>
        ...
        <div className="grid grid-cols-5 gap-4">
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-12 text-gray-500 mb-1 uppercase tracking-wide">{t('auctionsList.total')}</div>
            <div className="text-28 font-bold text-gray-900">{stats.total}</div>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-12 text-gray-500 mb-1 uppercase tracking-wide">{t('auctionsList.draft')}</div>
            <div className="text-28 font-bold text-gray-600">{stats.draft}</div>
          </div>
          <div className="bg-white rounded-lg border border-green-200 p-4">
            <div className="text-12 text-gray-500 mb-1 uppercase tracking-wide">{t('auctionsList.open')}</div>
            <div className="text-28 font-bold text-green-600">{stats.open}</div>
          </div>
          <div className="bg-white rounded-lg border border-yellow-200 p-4">
            <div className="text-12 text-gray-500 mb-1 uppercase tracking-wide">{t('auctionsList.locked')}</div>
            <div className="text-28 font-bold text-yellow-600">{stats.locked}</div>
          </div>
          <div className="bg-white rounded-lg border border-blue-200 p-4">
            <div className="text-12 text-gray-500 mb-1 uppercase tracking-wide">{t('auctionsList.cleared')}</div>
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 mb-8">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-red-800 font-semibold mb-1">{t('auctionsList.connectionError')}</p>
              ...
              >
                {t('auctionsList.retryConnection')}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        <div className="px-6 py-4 border-b border-gray-100">
          <div className="flex justify-between items-center">
            <h2 className="text-20 font-semibold">{t('auctionsList.allAuctions')}</h2>
            ...
            <div className="flex gap-2">
              {(['all', 'draft', 'open', 'locked', 'cleared', 'published'] as const).map((status) => (
                ...
                >
                  {status === 'all' ? `${t('auctionsList.all')} (${stats.total})` : t(`auctionsList.${status}`)}
                </button>
              ))}
            </div>
          </div>
          ...
          {sortedAuctions.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 font-medium mb-2">
                {error ? t('auctionsList.noConnection') : t('auctionsList.noAuctionsFound')}
              </p>
              <p className="text-14 text-gray-400 mb-4">
                {error
                  ? t('auctionsList.ensureApiRunning')
                  : statusFilter === 'all'
                    ? t('auctionsList.createFirstAuction')
                    : `${t('auctionsList.noAuctionsWithStatus')} "${t(`auctionsList.${statusFilter}`)}"`
                }
              </p>
              {!error && statusFilter === 'all' && (
                ...
                >
                  {t('auctionsList.createFirstAuctionBtn')}
                </Link>
              )}
            </div>
          ) : (
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          <h3 className="text-18 font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">
                            {auction.area} {t('auctionsList.auction')}
                          </h3>
                          <span className={`px-3 py-1 rounded-lg border text-12 font-medium ${getStatusBadge(auction.status)}`}>
                            {t(`auctionsList.${auction.status}`).toUpperCase()}
                          </span>
                          <span className="px-3 py-1 bg-gray-100 text-gray-700 rounded-lg text-12 font-mono">
                            {auction.mode === 'uniform_price' ? t('auctionsList.uniformPrice').toUpperCase() : t('auctionsList.payAsBid').toUpperCase()}
                          </span>
                        </div>

                        <div className="grid grid-cols-4 gap-6 text-14">
                          <div>
                            <div className="text-gray-500 text-12 mb-1">{t('auctionsList.auctionId')}</div>
                            <div className="font-mono text-gray-900">#{auction.auction_id}</div>
                          </div>
                          <div>
                            <div className="text-gray-500 text-12 mb-1">{t('auctionsList.biddingPeriod')}</div>
                            <div className="text-gray-900">
                              {formatDateTime(auction.starts_at).split(' ')[0]} - {formatDateTime(auction.ends_at).split(' ')[0]}
                            </div>
                          </div>
                          <div>
                            <div className="text-gray-500 text-12 mb-1">{t('auctionsList.created')}</div>
                            <div className="text-gray-900">{formatDateTime(auction.created_at).split(' ')[0]}</div>
                          </div>
                          {auction.cleared_price !== null && (
                            <div>
                              <div className="text-gray-500 text-12 mb-1">{t('auctionsList.clearedPrice')}</div>
      ...
      <div className="mt-6">
        ...
        >
          ...
          {t('auctionsList.backToDashboard')}
        </Link>
      </div>
    </>
  )
